...
class CodflowAmeexDiagnose extends Command
{
    ...
    public function handle(AmeexDeliveryService $ameex): int
    {
        ...
        $this->table(['Clé', 'Valeur'], [
            ...
            ['business_id (expéditeur)', $ameex->senderBusinessId($company) ?? '(manquant)'],
            ['hub_id (warehouse)', $ameex->hubId($company) ?? '(manquant)'],
            ['hub / expéditeur', $ameex->hubDisplayName($company) ?? $ameex->businessDisplayName($company) ?? '(nom inconnu — synchronisez les expéditeurs)'],
